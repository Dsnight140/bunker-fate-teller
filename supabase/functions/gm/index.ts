...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "scenario") {
      ...
      result = await callAI(
        SYSTEM,
        `Создай персонажа. Возраст: ВСЕГДА генерируй разный возраст (18-75 лет), НЕ повторяй 28 и другие уже использованные числа. Варьируй между молодыми (18-25), средними (26-50) и старшими (51-75).

КОНКРЕТИЗАЦИЯ ОБЯЗАТЕЛЬНА! Используй примеры:
- ПРОФЕССИЯ (конкретная специализация): врач-кардиолог, backend-разработчик, летчик, психолог, уголовник с судимостью, алкоголик в запое
- ЗДОРОВЬЕ (конкретный диагноз): астма (приступы при нагрузке), диабет 1 типа, полностью здоров, парализован, туберкулез, рак, слепота, гемофилия
- ФОБИЯ (конкретная иррациональная фобия НЕЗАВИСИМО от других характеристик): агорафобия, клаустрофобия, триффобия, боязнь высоты, арахнофобия, номофобия
- ХОББИ (конкретное занятие НЕЗАВИСИМО): легкая атлетика, альпинизм, компьютерные игры 12ч, наркотики, выживание, стрелковый спорт
- БАГАЖ (конкретный предмет с функцией): спасательный жилет, аптечка, компас, кирпич, долговая расписка, наркотики, ядовитое вещество
- ЧЕРТЫ (положительные И отрицательные, влияют на выживание): лидер, параноик, психопат, быстро решает, трус, садист

РАНДОМИЗМ: Случайно делай персонажа либо ПОЗИТИВНЫМ (хорошее здоровье, полезная профессия, хорошее хобби, полезный багаж) либо НЕГАТИВНЫМ (болезни, вредные привычки, мешающие фобии, бесполезный багаж).

Сложность: ${difficulty} (на лёгкой давай больше полезных способностей). Катастрофа: ${JSON.stringify(payload?.catastrophe)}. Ник: ${payload?.nickname}. 

ВАЖНО: Каждая характеристика НЕЗАВИСИМА - не связывай их между собой!`,
        characterSchema
      );
