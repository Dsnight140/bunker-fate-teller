...
function normalizeCharacter(character: any) {
  ...
  return {
    ...
    strengths: Array.isArray(character.strengths)
      ? character.strengths.map((t: string) => simplifyText(t, 10))
      : character.strengths,
    risks: Array.isArray(character.risks)
      ? character.risks.map((t: string) => simplifyText(t, 10))
      : character.risks,
  };
}
...
⚠️ Для каждого персонажа обязательно верни два массива:
- strengths: ровно 2 сильные стороны для этой катастрофы
- risks: ровно 2 слабые стороны/риска для этой катастрофы
...
2. У каждого персонажа должны быть как минимум 2 полезных и 2 рискованных качества.
3. strengths и risks формируй с учетом конкретной катастрофы.
4. ЧЕРТЫ ХАРАКТЕРА - 3-4 пункта, смешай положительные и отрицательные.
5. Профессия, багаж, хобби, фобия, здоровье, выживание, психология - независимы друг от друга.
...
ФОРМАТ JSON (точно такой):
{
  ...
  "characters": {
    ${players.map((p, idx) => {
      ...
      "strengths": ["сильная сторона 1 для катастрофы", "сильная сторона 2 для катастрофы"],
      "risks": ["риск 1 для катастрофы", "риск 2 для катастрофы"],
      "special_cards": [
